...
@Tag(name = "Clinical Forensic", description = "Endpoints for Medico-Legal Examination Form (MLEF) records")
public class ClinicalForensicController {
    ...
    @Operation(summary = "Create a new MLEF record")
    public ResponseEntity<MlefRecordDto> createMlefRecord(@Valid @RequestBody MlefRecordDto dto) {
    ...
    @Operation(summary = "Get all MLEF records")
    ...
    @GetMapping("/mlef/{id}")
    @PreAuthorize("hasAnyRole('JMO', 'MEDICAL_OFFICER', 'ADMIN')")
    @Operation(summary = "Get MLEF record by ID")
    ...
    @PutMapping("/mlef/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'JMO', 'MEDICAL_OFFICER')")
    @Operation(summary = "Update an existing MLEF record")
    public ResponseEntity<MlefRecordDto> updateMlefRecord(@PathVariable Long id, @Valid @RequestBody MlefRecordDto dto) {
        return ResponseEntity.ok(clinicalForensicService.updateMlefRecord(id, dto));
    }

    @PostMapping("/mlef/{id}/mlr-report")
    @PreAuthorize("hasAnyRole('ADMIN', 'JMO', 'MEDICAL_OFFICER')")
    @Operation(summary = "Generate an MLR report for an MLEF record")
    public ResponseEntity<MlrReportDto> generateMlrReport(@PathVariable Long id, @Valid @RequestBody MlrReportDto dto) {
